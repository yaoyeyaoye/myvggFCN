...
def get_model_data(dir_path, model_url):
    ...
    data = scipy.io.loadmat(filepath) # Returns: dictionary with variable names as keys, and loaded matrices as values.
    return data
...
def maybe_download_and_extract(dir_path, url_name, is_tarfile=False, is_zipfile=False):
    ...
    if not os.path.exists(filepath):
        ...
        filepath, _ = urllib.urlretrieve(url_name, filepath, reporthook=_progress)
        print()
...
def weight_variable(shape, stddev=0.02, name=None):
    '''to generate a normal distribution variable as weight'''
    # print(shape)
    ...
    # tf.Variable() 每次都在创建新对象，所有reuse=True 和它并没有什么关系
    # get_variable() 如果已经创建的变量对象，就把那个对象返回，如果没有创建变量对象的话，就创建一个新的


def bias_variable(shape, name=None):
    '''to generate a constant variable as bias'''
